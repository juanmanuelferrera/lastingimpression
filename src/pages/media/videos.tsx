import React from 'react';
import posts from '../../data/posts.json';
import { Container, Typography, List, ListItem, ListItemText, Divider, Box } from '@mui/material';
import Link from 'next/link';
import { useLanguage } from '../../components/LanguageContext';

function getSlugFromUrl(url: string) {
  if (!url) return '';
  const match = url.match(/\/([^\/]+)\/?$/);
  return match ? match[1].replace(/\..*$/, '') : '';
}

const VideosPage = () => {
  const { language } = useLanguage();
  const videoPosts = posts.filter(post => post.language === language && post.videos && post.videos.length > 0);
  return (
    <Container maxWidth="md" sx={{ py: 6 }}>
      <Typography variant="h3" component="h1" fontWeight={700} gutterBottom>
        Videos
      </Typography>
      {videoPosts.length === 0 ? (
        <Typography variant="body1" color="text.secondary">No video posts found for the selected language.</Typography>
      ) : (
        <List>
          {videoPosts.map((post, idx) => (
            <React.Fragment key={post.title + post.date}>
              <ListItem alignItems="flex-start" component={Link} href={`/${getSlugFromUrl(post.url || '')}`} button>
                <ListItemText
                  primary={<Typography variant="h6" fontWeight={600}>{post.title}</Typography>}
                  secondary={
                    <>
                      <Typography variant="caption" color="text.secondary">
                        {post.date ? new Date(post.date).toLocaleDateString() : ''}
                      </Typography>
                      <Box sx={{ mt: 1 }}>
                        {post.videos.map((src: string, i: number) => (
                          <Box key={i} sx={{ my: 1 }}>
                            <iframe src={src} width="100%" height="300" allowFullScreen style={{ border: 0 }} />
                          </Box>
                        ))}
                      </Box>
                    </>
                  }
                />
              </ListItem>
              {idx < videoPosts.length - 1 && <Divider />}
            </React.Fragment>
          ))}
        </List>
      )}
    </Container>
  );
};

export default VideosPage; 